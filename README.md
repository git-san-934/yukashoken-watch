...
- 本番実行(約3,800社規模)で、EDINET側のレート制限(HTTPステータスは200だが、応答本文が `{"StatusCode":"429","message":"Too Many Requests"}` というJSON)により事業内容の抽出が半数近く失敗する事象を確認しました。対策として、同時実行数(`BUSINESS_DESCRIPTION_CONCURRENCY`)を10→3に引き下げ、かつ429を検知した場合は指数バックオフで自動リトライする仕組みを `fetchDocumentCsvZip`(`src/lib/edinet.ts`)に実装済みです。書類そのものが存在しない場合の404はリトライ対象外です(待っても解決しないため)
