...
- 各書類に「PDFを見る」リンクを表示し、EDINET上の実際のPDF(`https://disclosure2dl.edinet-fsa.go.jp/searchdocument/pdf/{docID}.pdf` — APIキー不要でブラウザから直接閲覧できる、EDINET公式の書類配信用URL)にそのまま飛べます
...
- 【事業の内容】のテキストは、**各上場企業の最新の有価証券報告書1件のみ**を対象に抽出します。1つの書類の内容は公開後変わらないため、`data/business-descriptions.json` に **docID(書類管理番号)をキーに永続キャッシュ**しており、一度抽出した書類は次回以降フェッチし直しません。実行のたびにこのキャッシュをリポジトリへコミット・プッシュ(要 `contents: write` 権限。コミットメッセージに `[skip ci]` を付け、このpushが同じワークフローを再トリガーしないようにしています)し、次回実行時の起点にします。そのため**初回実行(約4,000社分の総ざらい)だけ重く**、2回目以降は「前回からEDINETに新規提出された会社の分だけ」を追加取得すれば済みます。キャッシュは実行ごとに、その時点で全上場企業に該当するdocIDだけへ刈り込み(廃止・上場廃止等で不要になった古いdocIDのエントリは自動的に消えます)、無制限に肥大化しないようにしています
- 本番実行(約3,800社規模、当時はまだ上記の永続キャッシュ導入前)で、EDINET側のレート制限(HTTPステータスは200だが、応答本文が `{"StatusCode":"429","message":"Too Many Requests"}` というJSON)により事業内容の抽出が半数近く失敗する事象を確認しました。対策として、同時実行数(`BUSINESS_DESCRIPTION_CONCURRENCY`)を10→3に引き下げ、かつ429を検知した場合は指数バックオフで自動リトライする仕組みを `fetchDocumentCsvZip`(`src/lib/edinet.ts`)に実装済みです。書類そのものが存在しない場合の404はリトライ対象外です(待っても解決しないため)。上記の永続キャッシュにより、2回目以降の実行はそもそも新規分だけしかリクエストしないため、レート制限にはさらに引っかかりにくくなっています
- CSVパッケージのZIP内部構成(エンコーディング・列名等)はEDINETの公開仕様に基づく実装です。実際のAPIキーでの本番稼働で、直近30日分1,600件超の取得・うち直近3日分の財務数値抽出(60件超)がほぼ問題なく動作することを確認済みです(`src/lib/edinetFinancials.ts` 参照)。事業内容テキストの抽出(`DescriptionOfBusinessTextBlock` 要素・約4,000件規模)についても、上記のレート制限対策を反映した実データでの動作を今後のデプロイで確認していきます

## 技術構成

- Next.js (App Router) + TypeScript, `output: "export"` による静的書き出し
- Tailwind CSS
- 訪問者向けにはサーバー・DBを持たない。訪問のたびにサーバー側の最新スナップショットを取得して表示するだけで、ブラウザ側に永続保存するデータはありません。ビルド時のみ、事業内容の抽出結果をリポジトリ内の `data/business-descriptions.json` に永続キャッシュしています(上記「EDINET データについて」参照)
